...
Ark::Value sf_window_init(std::vector<Ark::Value>& n,
                          [[maybe_unused]] Ark::VM* vm)
{
    if (n.size() != 3)
        throw std::runtime_error(
            "sf:window:init needs 3 arguments: width, height and title");
    ...
    if (!get_has_window())
    {
        get_window().create(sf::VideoMode(static_cast<long>(n[0].number()),
                                          static_cast<long>(n[1].number())),
                            n[2].string().toString());
        get_has_window() = true;
    }
    else
...
Ark::Value sf_window_isopen([[maybe_unused]] std::vector<Ark::Value>& n,
                            [[maybe_unused]] Ark::VM* vm)
{
    return get_window().isOpen() ? Ark::True : Ark::False;
}

Ark::Value sf_window_clear(std::vector<Ark::Value>& n,
                           [[maybe_unused]] Ark::VM* vm)
{
    if (n.size() != 3)
    ...
    get_window().clear(sf::Color(static_cast<long>(n[0].number()),
                                 static_cast<long>(n[1].number()),
                                 static_cast<long>(n[2].number())));
    return Ark::Nil;
}

Ark::Value sf_window_display([[maybe_unused]] std::vector<Ark::Value>& n,
                             [[maybe_unused]] Ark::VM* vm)
{
    get_window().display();
    return Ark::Nil;
...
Ark::Value sf_window_set_fps(std::vector<Ark::Value>& n,
                             [[maybe_unused]] Ark::VM* vm)
{
    if (n[0].valueType() != Ark::ValueType::Number)
...
Ark::Value sf_window_close([[maybe_unused]] std::vector<Ark::Value>& n,
                           [[maybe_unused]] Ark::VM* vm)
{
    get_window().close();
    return Ark::Nil;
...
Ark::Value sf_draw(std::vector<Ark::Value>& n, [[maybe_unused]] Ark::VM* vm)
{
