...
Ark::Value sf_load_texture(std::vector<Ark::Value>& n, Ark::VM* vm)
{
    if (n.size() != 1)
    ...
    return Ark::Value(UserType(&texture));
}

Ark::Value sf_load_sprite(std::vector<Ark::Value>& n, Ark::VM* vm)
{
    if (n.size() != 1)
        throw std::runtime_error("sf:load:sprite: need 1 argument: texture");
    if (n[0].valueType() != Ark::ValueType::User || !n[0].usertypeRef().is<sf::Texture>())
    ...
    return Ark::Value(UserType(&object));
}

Ark::Value sf_load_font(std::vector<Ark::Value>& n, Ark::VM* vm)
{
    if (n.size() != 1)
    ...
    return Ark::Value(UserType(&font));
}

Ark::Value sf_make_text(std::vector<Ark::Value>& n, Ark::VM* vm)
{
    if (n.size() != 4)
        throw std::runtime_error("sf:text:make: need 4 arguments: font, text, size, color");
    if (n[0].valueType() != Ark::ValueType::User || !n[0].usertypeRef().is<sf::Font>())
    ...
    object.setFillColor(sf::Color(
        static_cast<long>(n[3].constList()[0].number()),
...
Ark::Value sf_set_text(std::vector<Ark::Value>& n, Ark::VM* vm)
{
    if (n.size() != 2)
        throw std::runtime_error("sf:text:set: need 2 arguments: text object, new value");
    if (n[0].valueType() != Ark::ValueType::User || !n[0].usertypeRef().is<sf::Text>())
...
Ark::Value sf_setpos(std::vector<Ark::Value>& n, Ark::VM* vm)
{
    if (n.size() != 3)
    ...
    if (n[0].usertypeRef().is<sf::Text>())
        n[0].usertypeRef().as<sf::Text>().setPosition(static_cast<long>(n[1].number()), static_cast<long>(n[2].number()));
    else if (n[0].usertypeRef().is<sf::Sprite>())
        n[0].usertypeRef().as<sf::Sprite>().setPosition(static_cast<long>(n[1].number()), static_cast<long>(n[2].number()));
    else
...
Ark::Value sf_width(std::vector<Ark::Value>& n, Ark::VM* vm)
{
    if (n.size() != 1)
    ...
    if (n[0].usertypeRef().is<sf::Text>())
        return Ark::Value(static_cast<int>(n[0].usertypeRef().as<sf::Text>().getGlobalBounds().width));
    else if (n[0].usertypeRef().is<sf::Sprite>())
        return Ark::Value(static_cast<int>(n[0].usertypeRef().as<sf::Sprite>().getGlobalBounds().width));
    throw Ark::TypeError("Object isn't a SFML object");
}

Ark::Value sf_height(std::vector<Ark::Value>& n, Ark::VM* vm)
{
    if (n.size() != 1)
    ...
    if (n[0].usertypeRef().is<sf::Text>())
        return Ark::Value(static_cast<int>(n[0].usertypeRef().as<sf::Text>().getGlobalBounds().height));
    else if (n[0].usertypeRef().is<sf::Sprite>())
        return Ark::Value(static_cast<int>(n[0].usertypeRef().as<sf::Sprite>().getGlobalBounds().height));
    throw Ark::TypeError("Object isn't a SFML object");
}